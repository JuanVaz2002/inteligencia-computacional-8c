...
# Default configs
DEFAULT_DATA_DIR = "./samples/rag/data"
...
def expand_query(query: str, llm_model: str, client: OpenAI) -> list[str]:
    """Generate several phrasing of the original question for sematic searches"""

    EXPANSION_QUERY_PROMPT =  f"""Question: {query}
    
                                  For semantic search, generate 3 alternative sentences of this question. 
                                  Return only the questions, one per line."""

    response = client.chat.completions.create(
        model=llm_model,
        messages=[{"role": "user", "content": EXPANSION_QUERY_PROMPT}],
        )
    
    variants = response.choices[0].message.content.strip().split("\n")
    expansion = [query] + variants
    return expansion # original + expansions
...
def retrieve(
    ...
    """
    # Generar las frases de preguntas 
    query_expansion = expand_query(query, llm_model, client)

    #convertir la pregunta a un embedding
    query_embedding = model.encode(query_expansion, normalize_embeddings=True)
...
class Assistant:
    ...
    def ask(self, question: str, k: int | None = None) -> str:
        ...
        retrieved_docs = retrieve(search_query, self.index, self.model, self.chunks, self.client, self.llm_model, search_k, filter_type=filter_type)
